...
ngx_int_t
healthcheck_https_helper::make_request(ngx_dynamic_healthcheck_opts_t *shared,
...
ngx_int_t
healthcheck_https_helper::parse_status_line(ngx_dynamic_hc_local_node_t *state)
{
    if (status.code != 0)
        return NGX_OK;
...
ngx_int_t
healthcheck_https_helper::receive_data(ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;
    ngx_buf_t         *buf = state->buf;
    ssize_t            size;
    ...
    ngx_log_error(NGX_LOG_DEBUG, c->log, 0,
                  "[%V] %V: %V addr=%V, "
                  "fd=%d https on_recv() recv: %d, eof=%d, pending_eof=%d",
...
ngx_int_t
healthcheck_https_helper::parse_headers(ngx_dynamic_hc_local_node_t *state)
{
    ngx_keyval_t  h;
...
ngx_int_t
healthcheck_https_helper::parse_body_chunked(ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;
    ssize_t            size;
    u_char            *sep;
...
ngx_int_t
healthcheck_https_helper::parse_body(ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;
    ngx_buf_t         *buf = state->buf;
...
ngx_int_t
healthcheck_https_helper::receive_body(ngx_dynamic_healthcheck_opts_t *shared,
    ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;
    ...
    ngx_log_error(NGX_LOG_DEBUG, c->log, 0,
                  "[%V] %V: %V addr=%V, fd=%d https receive_body()",
                  &module, &upstream, &server, &name, c->fd);
    ...
    if (pool == NULL) {
        ...
        ngx_log_error(NGX_LOG_WARN, c->log, c->fd, 0,
                      "[%V] %V: %V addr=%V, fd=%d https receiving body: "
...
ngx_int_t
healthcheck_https_helper::receive_headers(ngx_dynamic_healthcheck_opts_t *shared,
    ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;

    ngx_log_error(NGX_LOG_DEBUG, c->log, 0,
                  "[%V] %V: %V addr=%V, fd=%d https receive_headers()",
                  &module, &upstream, &server, &name, c->fd);
...
receive:
        ...
        if (eof) {
            ...
            ngx_log_error(NGX_LOG_WARN, c->log, 0,
                          "[%V] %V: %V addr=%V, fd=%d https connection"
...
ngx_int_t
healthcheck_https_helper::receive(ngx_dynamic_healthcheck_opts_t *shared,
    ngx_dynamic_hc_local_node_t *state)
{
    ngx_connection_t  *c = state->pc.connection;
    ngx_int_t          rc;
    ...
    ngx_log_error(NGX_LOG_DEBUG, state->pc.connection->log, 0,
                  "[%V] %V: %V addr=%V, fd=%d https on_recv() %s",
                  &module, &upstream, &server, &name, c->fd,
    ...
    if (shared->response_codes.len) {
        ...
        if (j == shared->response_codes.len) {
            ...
            ngx_log_error(NGX_LOG_WARN, c->log, 0,
                          "[%V] %V: %V addr=%V, fd=%d https status "
    ...
    if (shared->response_body.len) {
        ...
        switch(ngx_dynamic_healthcheck_match_buffer(&shared->response_body,
            ...
            case NGX_OK:
                ...
                ngx_log_error(NGX_LOG_DEBUG, c->log, 0,
                              "[%V] %V: %V addr=%V, fd=%d https pattern"
                              " '%V' found",
                              &module, &upstream, &server, &name, c->fd,
...
healthcheck_https_helper::~healthcheck_https_helper()
{
    if (pool != NULL)
